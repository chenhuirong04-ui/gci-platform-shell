...
Rules:
- CRITICAL — explicit new-customer creation phrases with no name given: patterns like "帮我建个客户" / "帮我登记一个新客户" / "新建一个客户" / "add a new customer" / "create a customer" that ask to create a customer but do NOT state a name must still be classified as NEW_CUSTOMER (never UNKNOWN/LOOKUP just because the name is missing) — set customer_name: null in that case. The caller asks Chris for the name before writing anything.
