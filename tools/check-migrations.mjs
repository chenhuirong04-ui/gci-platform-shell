...
for (const f of modern) {
  ...
  for (const m of sql.matchAll(/\b(insert\s+into|delete\s+from|update)\s+(?:only\s+)?("?[a-z_][a-z0-9_]*"?(?:\."?[a-z_][a-z0-9_]*"?)?)/gi)) {
    ...
    // trigger event clause (CREATE TRIGGER … BEFORE UPDATE ON t / UPDATE OF col) or policy command
    // (CREATE POLICY … FOR UPDATE TO role), not an UPDATE statement
    if (/^(update)$/i.test(m[1]) && /^(on|of|to)$/i.test(target)) continue;
